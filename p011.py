...
for y in range(0, 17):
    for x in range(0, 20):
        if (grid[y][x] * grid[y+1][x] * grid[y+2][x] * grid[y+3][x]) > largest:
            largest = grid[y][x] * grid[y+1][x] * grid[y+2][x] * grid[y+3][x]
            
# left/ right
for y in range(0, 20):
    for x in range(0, 17):
        if (grid[y][x] * grid[y][x+1] * grid[y][x+2] * grid[y][x+3]) > largest:
            largest = grid[y][x] * grid[y][x+1] * grid[y][x+2] * grid[y][x+3]

# diagonal
for y in range(0, 17):
    for x in range(0, 17):
        if (grid[y][x] * grid[y+1][x+1] * grid[y+2][x+2] * grid[y+3][x+3]) > largest:
            largest = grid[y][x] * grid[y+1][x+1] * grid[y+2][x+2] * grid[y+3][x+3]
